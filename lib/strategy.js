...
function OAuth2Strategy(options, verify) {
  ...
  this._scope = options.scope.split(this._scopeSeparator);
    ...
    function loaded(err, ok, state) {
      ...
      self._oauth2.getOAuthAccessToken(code, params,
        function(err, accessToken, refreshToken, params) {
          ...
          const selfScope = self._scope;
          const paramScope = params['scope'].split(self._scopeSeparator);
          const isCorrectScope = () => {
            for (const scope of selfScope) {
                if (!Boolean(paramScope.find(v => v === scope))) return false;
            }
            return true;
          };
          if (selfScope.length !== paramScope.length || !isCorrectScope()) { return self.error(['SCOPE_MISMATCH', [paramScope, selfScope]]); };

          if (!params['expires_in'] || isNaN(Number(params['expires_in']))) return self.error(self._createOAuthError('expires_in is missing', new Error('key "expires_in" is missing')));
          ...
          self._loadUserProfile(accessToken, function(err, profile) {
            ...
            try {
				        ...
				        profile.expires = new Date(profile.fetchedAt.getTime() + params['expires_in'] * 1000);
